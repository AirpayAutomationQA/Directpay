...
public class Create_TestNGXML {	
    ...
    public void createXMLfile () throws Exception {
		 ...
		 * //killProcessRunning("iexplore.exe *32"); killProcessRunning("iexplore.exe");
		 * killProcessRunning("ALM-Client.exe"); killProcessRunning("chromedriver.exe");
		 * killProcessRunning("chrome.exe"); killProcessRunning("scalc.exe");
		 */	
	 ...
	 * 
	 * Process p = Runtime.getRuntime().exec(TASKLIST); BufferedReader reader = new
	 * BufferedReader(new InputStreamReader(p.getInputStream())); String line; while
	 * ((line = reader.readLine()) != null) { if (line.contains(serviceName)) {
	 * Runtime.getRuntime().exec(KILL+serviceName); flag= true; } } }
