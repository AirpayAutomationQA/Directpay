...
public class AirPay_PaymentPage_BusinessLogic extends Airpay_PaymentPage_PageObject {
		...
		try{ 
			...
			{         	
				...
				{				
					WebElement selectDropBox1 = driver.findElement(By.xpath(Netbank_DropDown));
					Select select1 =new Select(selectDropBox1);
					selectDropBox1.getSize();
					//System.out.println(selectDropBox1.getSize());
	...
	public void Verify_PaymentPageFields() throws Exception {
		try{ 
			...
			{         	
				...
				Extent_Reporting.Log_report_img("Logo exist", "Passed", driver);
			}else{
				Extent_Reporting.Log_Fail("Logo payment page does not exist",	"Failed",driver);
				Log.error("Local Host page not successfully displayed");
				throw new Exception("option does not exist displayed");
			}
		}                     
	...
	public void Verify_All_Channels() throws Exception {
		...
		{
			Extent_Reporting.Log_Fail("Channels does not exist",	"Failed",driver);
	...
	public void SummerSectionVerify_indianCurrency() throws Exception {
		try{ 
			...
			{			
			  ...
			  //OrderID.trim().equalsIgnoreCase(GetOrderID && 
				if(Amtval[1].trim().equalsIgnoreCase(Excel_Handling.Get_Data(TC_ID, "Amount").trim())){
				...
				Extent_Reporting.Log_Fail("Summery section does not exist", "Failed",driver);
				throw new Exception("There is an something issue with order id and amount summery");
			}			
		}                     
		catch(Exception e)	
		{
			...
			Log.error("Channenls does not exist");
			e.printStackTrace();
			throw new Exception("Channels does not exist");
		}
	}
	...
	public void BankPage_validation() throws Exception {
		try{ 
			...
			if(domain.equals("") || domain.equals("payments.airpay.co.in")||(driver.getPageSource().contains("Error Page Exception"))==true
					...
					driver.getCurrentUrl().equalsIgnoreCase("https://payments.airpay.co.in/error.php"))
			{			 
				Extent_Reporting.Log_Fail("Its not navigated to Respective Bank or redirected to invalid url.", "Error Snap", driver);
				Log.error("Its not navigated to Respective Bank or redirected to invalid url. "+bankName+"  "+driver.getCurrentUrl());
			}else{
	...
	public void NavigateToLocalHostPage() throws Exception {
		try{ 
			...
			Thread.sleep(2500);
			driver.getCurrentUrl();
			if(driver.getCurrentUrl().equalsIgnoreCase("https://directpayments.nowpay.co.in"))
			{	
				Thread.sleep(2500);
				Assert.waitForPageToLoad(driver);
				LocalHostDetailPage();	
				...
				{
					Extent_Reporting.Log_Fail("Error occured for bank", "Bank is not displayed", driver);   
					Extent_Reporting.Log_report_img(bankName, "Bank is failed to displayed", driver);
			...
			else
			{
				...
				{
					driver.navigate().back();
					driver.get("https://directpayments.nowpay.co.in/#");
					Thread.sleep(15000);
					Assert.waitForPageToLoad(driver);
					LocalHostDetailPage();	
					Card_Details_Options();
