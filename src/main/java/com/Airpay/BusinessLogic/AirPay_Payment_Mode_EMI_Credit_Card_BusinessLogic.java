...
public class AirPay_Payment_Mode_EMI_Credit_Card_BusinessLogic extends Airpay_PaymentPage_PageObject {
	...
	public void AmexZeClick() throws Exception{
		try{		   		   			
			...
			if(driver.getCurrentUrl().equalsIgnoreCase("https://payments.airpay.co.in/pay/directindexapi.php"))
			{
				String heading=driver.findElement(By.xpath("//h1")).getText();
				String paraerror1=driver.findElement(By.xpath("//p[1]")).getText();
				String paraerror2=driver.findElement(By.xpath("//p[2]/text()")).getText();
				if(heading.equalsIgnoreCase("Forbidden")
						||paraerror1.equalsIgnoreCase("You don't have permission to access /pay/directindexapi.php\r\n" + 
								"on this server.")
						||paraerror2.equalsIgnoreCase("Additionally, a 403 Forbidden\r\n" + 
								"error was encountered while trying to use an ErrorDocument to handle the request."))
				{
					Extent_Reporting.Log_Fail("AmexeZeClick is not working", "Failed", driver);
				}
			}
			else
			{
				Extent_Reporting.Log_report_img("Amazon_pay is exist as expected", "Passed", driver);
				Extent_Reporting.Log_Pass("AmexeeZeClick site is launch", "AmexeZeclick link is: "+driver.getCurrentUrl());
			}
		Assert.waitForPageToLoad(driver);
		}catch(Exception e)	
		{
	...
	public void AmexeZeClickNavigation_page() throws Exception {
		try{ 
			...
			JavascriptExecutor js = (JavascriptExecutor) driver;
			  ...
			  {			 
				  Extent_Reporting.Log_Fail("Its not navigated to Respective URL as", "Error Snap", driver);
				  Log.error("Its not navigated to Respective URL as :"+driver.getCurrentUrl());
				//throw new Exception("Net Banking page issue");
			  }else{
				  Extent_Reporting.Log_Pass("Its Navigated to :"+driver.getCurrentUrl(), "Passed");
