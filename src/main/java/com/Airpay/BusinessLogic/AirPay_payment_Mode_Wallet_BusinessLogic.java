...
public class AirPay_payment_Mode_Wallet_BusinessLogic extends Airpay_PaymentPage_PageObject {
   ...
   public static String OtpPass =null;
	...
	public void WalletEmail_MobileNumber_verifying() throws Exception {
		try{ 
			...
			JavascriptExecutor js = (JavascriptExecutor) driver;
			  ...
			  {
				  Extent_Reporting.Log_Fail("Incorrect URL, Verify same manually", "Failed", driver);
				  Extent_Reporting.Log_report_img("Incorrect URL, Verify same manually" , "Failed", driver);
			  }
			  else
			  if(domain.equals("") || domain.equals("payments.airpay.co.in")||driver.getPageSource().contains("Error Page Exception")==true
					  ||driver.getPageSource().contains("Internal Server Error")==true|| driver.getTitle().contains("HTTP Status - 400"))
	...
	public void LocalHostDetailPage() throws Exception {
		try{ 
			Log.info("Navigating To direct payment page");	   
	...
	public void NavigateToLocalHostPage() throws Exception {
		try{ 
				...
				if(driver.getCurrentUrl().equalsIgnoreCase("https://payments.airpay.co.in/error.php"))
				{
					Extent_Reporting.Log_Fail("Wallet selection", "Wallet is not selected, kindly rectify manually. ", driver);
					Extent_Reporting.Log_report_img("Wallet selection", "Wallet not selected.", driver);
				}
				driver.navigate().back();
				if (driver.getCurrentUrl().equalsIgnoreCase("https://directpayments.nowpay.co.in/#")||
						driver.getCurrentUrl().equalsIgnoreCase("https://directpayments.nowpay.co.in/"))
				{
					Assert.waitForPageToLoad(driver);
					LocalHostDetailPage();	
					...
					if (driver.getCurrentUrl().equalsIgnoreCase("https://directpayments.nowpay.co.in/#")==false &&
							driver.getCurrentUrl().equalsIgnoreCase(Excel_Handling.Get_Data(TC_ID, "PaymentPage_URL").trim())==false)
					{
						driver.navigate().refresh();
						Thread.sleep(1500);
						driver.navigate().back();
						
						if (driver.getCurrentUrl().equalsIgnoreCase(Excel_Handling.Get_Data(TC_ID, "PaymentPage_URL").trim())==false)
							{
								Thread.sleep(1500);
